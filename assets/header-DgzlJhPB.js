(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))c(o);new MutationObserver(o=>{for(const s of o)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&c(r)}).observe(document,{childList:!0,subtree:!0});function n(o){const s={};return o.integrity&&(s.integrity=o.integrity),o.referrerPolicy&&(s.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?s.credentials="include":o.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function c(o){if(o.ep)return;o.ep=!0;const s=n(o);fetch(o.href,s)}})();const l="exercise-card",R=10,i=Object.freeze({START:"start",RATE:"rate",REMOVE_FAVORITE:"removeFavorite"});function p(e,t){return`
  ...
  `}function g(e,t){const n=e.querySelector(`.${l}[data-id="${t}"]`);n&&n.remove()}const m="favoriteExercises";function v(e){const n=h().filter(c=>c._id!==e);E(n)}function E(e){localStorage.setItem(m,JSON.stringify(e))}function h(){try{const e=localStorage.getItem(m);if(e){const t=JSON.parse(e);if(Array.isArray(t))return t.filter(y)}}catch{}return[]}function y(e){return e&&typeof e=="object"&&typeof e._id=="string"&&typeof e.name=="string"}function b(e,t){v(t),g(e,t)}function S(e){console.log(`Rating exercise: ${e}`)}function L(e){console.log(`Starting exercise: ${e}`)}const a=document.querySelector(".exercises-list-wrapper");function T(e,t,{favorites:n,onFavoriteDelete:c}={}){a.innerHTML="",n&&a.classList.add("favorites");let o="";e.length?e.forEach(s=>{o+=p(s,n)}):o=`<p class="no-exercises-message">${t}</p>`,a.insertAdjacentHTML("beforeend",o),a.onclick=s=>{let r=s.target;if(r.nodeName!=="BUTTON"&&(r=r.closest(`.${l} button`)),!r||r.nodeName!=="BUTTON")return;const d=r.closest(`.${l}`).dataset.id,f=r.dataset.action;switch(f){case i.REMOVE_FAVORITE:b(a,d),c==null||c(d);break;case i.START:L(d);break;case i.RATE:S(d);break;default:console.warn("Unknown action:",f)}}}async function u(e,t){if(e==="open"){const n=x();document.body.classList.add("modal-open"),t.classList.remove("hidden"),t.classList.add("is-open"),document.documentElement.style.setProperty("--scrollbar-width",`${n}px`)}e==="close"&&(t.classList.remove("is-open"),setTimeout(()=>{t.classList.add("hidden"),document.body.classList.remove("modal-open"),document.documentElement.style.removeProperty("--scrollbar-width")},250))}const x=()=>window.innerWidth-document.documentElement.clientWidth;function O(){const e=document.querySelector(".mobile-menu-overlay"),t=document.querySelector(".burger-btn"),n=document.querySelector(".mobile-menu__close-button");e&&t&&n&&(t.addEventListener("click",()=>u("open",e)),n.addEventListener("click",()=>u("close",e)),e.addEventListener("click",({target:s})=>{(s.classList.contains("mobile-menu__nav-link")||s===e)&&u("close",e)}));const c=document.querySelectorAll(".nav-link"),o=window.location.pathname;c.forEach(s=>{new URL(s.href).pathname===o?s.classList.add("active"):s.classList.remove("active")})}export{R as E,h as g,O as l,T as r};
//# sourceMappingURL=header-DgzlJhPB.js.map
